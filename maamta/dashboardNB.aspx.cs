...
namespace maamta
{
    public partial class dashboardNB : System.Web.UI.Page
    {
        ...
        private void ShowData()
        {
            ...
            {
                con.Open();
                MySqlCommand cmd = new MySqlCommand("(select a.Site,a.name as Tab_User,(select count(*)  from view_crf4b as x where a.name=x.name  and (str_to_date(x.lw_crf4b_2, '%d-%m-%Y') between str_to_date('" + txtCalndrDate.Text + "', '%d-%m-%Y') and str_to_date('" + txtCalndrDate1.Text + "', '%d-%m-%Y')) ) as  TotalApporach,(select count(*)  from view_crf4b as x where a.name=x.name  and x.q19!='Complete' and (str_to_date(x.lw_crf4b_2, '%d-%m-%Y') between str_to_date('" + txtCalndrDate.Text + "', '%d-%m-%Y') and str_to_date('" + txtCalndrDate1.Text + "', '%d-%m-%Y')) ) as  Incomplete_CRF4,(select count(*)  from view_crf4b as x where a.name=x.name  and x.q19='Complete' and (str_to_date(x.lw_crf4b_2, '%d-%m-%Y') between str_to_date('" + txtCalndrDate.Text + "', '%d-%m-%Y') and str_to_date('" + txtCalndrDate1.Text + "', '%d-%m-%Y')) ) as  Complete_CRF4,(select count(*)  from view_crf5a as x where a.name=x.name  and (str_to_date(x.lw_crf5a_02, '%d-%m-%Y') between str_to_date('" + txtCalndrDate.Text + "', '%d-%m-%Y') and str_to_date('" + txtCalndrDate1.Text + "', '%d-%m-%Y')) ) as  Complete_CRF5a from (select cc.site,aa.name,bb.team_title_id from emp  as aa inner join team as bb on aa.team_id=bb.team_id inner join site as cc on cc.site_id=bb.site_id where team_title_id=3 and aa.access_status=1 order by Site)  as a ) union all (select '','Total',(select count(*)  from view_crf4b as x where (str_to_date(x.lw_crf4b_2, '%d-%m-%Y') between str_to_date('" + txtCalndrDate.Text + "', '%d-%m-%Y') and str_to_date('" + txtCalndrDate1.Text + "', '%d-%m-%Y')) ) as  TotalApporach_CRF4b,(select count(*)  from view_crf4b as x where  x.q19!='Complete' and (str_to_date(x.lw_crf4b_2, '%d-%m-%Y') between str_to_date('" + txtCalndrDate.Text + "', '%d-%m-%Y') and str_to_date('" + txtCalndrDate1.Text + "', '%d-%m-%Y')) ) as  IncompleteCRF4,(select count(*)  from view_crf4b as x where  x.q19='Complete' and (str_to_date(x.lw_crf4b_2, '%d-%m-%Y') between str_to_date('" + txtCalndrDate.Text + "', '%d-%m-%Y') and str_to_date('" + txtCalndrDate1.Text + "', '%d-%m-%Y')) ) as  Complete_CRF4, (select count(*)  from view_crf5a as x where (str_to_date(x.lw_crf5a_02, '%d-%m-%Y') between str_to_date('" + txtCalndrDate.Text + "', '%d-%m-%Y') and str_to_date('" + txtCalndrDate1.Text + "', '%d-%m-%Y')) ) as  TotalFilled_CRF5a)", con);             
