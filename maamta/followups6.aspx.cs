...
namespace maamta
{
    public partial class followups6 : System.Web.UI.Page
    {
            ...
            try
            {
                if (CheckBox1.Checked == true)
                {
                    if (DropDownList1.SelectedValue == "0")
                    {
                        con.Open();
                        MySqlCommand cmd;
                        cmd = new MySqlCommand("select * from view_followups6 as a left join participant_status as b on  a.study_code=b.study_code     where a.status!='1' and a.dssid like '%" + txtdssid.Text + "%'order by str_to_date(a.date, '%d-%m-%Y'),a.study_code,a.age;", con);
                    ...
                    else
                    {
                        con.Open();
                        MySqlCommand cmd;
                        cmd = new MySqlCommand("select * from view_followups6 as a left join participant_status as b on  a.study_code=b.study_code     where a.site='" + DropDownList1.SelectedValue + "' and a.status!='1' and a.dssid like '%" + txtdssid.Text + "%'order by str_to_date(a.date, '%d-%m-%Y'),a.study_code,a.age;", con);
                        MySqlDataAdapter sda = new MySqlDataAdapter();
                        {
                            cmd.Connection = con;
                            sda.SelectCommand = cmd;
        ...
        public void ExcelExport()
        {
            ...
            {
                Response.Clear();
                Response.AddHeader("content-disposition", "attachment;filename=Anthro-Followups Pending (" + DateTime.Today.ToString("dd-MM-yyyy") + ").xls");
                Response.Charset = "";
