...
namespace maamta
{
    public partial class showcrf5b : System.Web.UI.Page
    {
        ...
        private void Exportdata()
        {
            ...
            try
            {
                if (CheckBox1.Checked == false)
                {
                    con.Open();
                    MySqlCommand cmd;
                    cmd = new MySqlCommand("select * from view_crf5b_only WHERE DSSID LIKE '%" + txtdssid.Text + "%' and (str_to_date(date_of_attempt, '%d-%m-%Y') between str_to_date('" + txtCalndrDate.Text + "', '%d-%m-%Y') and str_to_date('" + txtCalndrDate1.Text + "', '%d-%m-%Y'))  order by str_to_date(date_of_attempt, '%d-%m-%Y'), STR_TO_DATE(time_of_attempt,  '%H:%i')", con);
                ...
                else
                {
                    con.Open();
                    MySqlCommand cmd;
                    cmd = new MySqlCommand("select * from view_crf5b_only WHERE DSSID LIKE '%" + txtdssid.Text + "%' order by str_to_date(date_of_attempt, '%d-%m-%Y'), STR_TO_DATE(time_of_attempt,  '%H:%i')", con);
                    MySqlDataAdapter sda = new MySqlDataAdapter();
                    {
                        cmd.Connection = con;
                        sda.SelectCommand = cmd;
        ...
        public void ExcelExport()
        {
            ...
            {
                Response.Clear();
                Response.AddHeader("content-disposition", "attachment;filename=FoodRecall CRF5b (" + DateTime.Today.ToString("dd-MM-yyyy") + ").xls");
                Response.Charset = "";
        ...
        protected void Link_Study(object sender, EventArgs e)
        {
            ...
            string form_crf_5b_id = commandArgs[0];
            string StudyId = commandArgs[1];

            Session["form_crf_5b_id"] = form_crf_5b_id;
            Session["StudyId_CRF5b"] = StudyId;
            Response.Redirect("showcrf5bbyid.aspx");
        }
    }
}